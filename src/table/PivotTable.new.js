...
export const PivotTable = function(refs, layout, response, colAxis, rowAxis, options = {}) {
    ...
    //TODO: add legend set to options
...
PivotTable.prototype.setViewportWidth = function(width=window.innerWidth) {
    this.viewportWidth = width;
};

PivotTable.prototype.setViewportHeight = function(height=window.innerHeight) {
...
PivotTable.prototype.setColumnEnd = function(index) {
    this.columnEnd = this.constrainWidth(Math.min(this.columnSize + rowAxis.dims - 1, index));
};

PivotTable.prototype.setRowEnd = function(index) {
    this.rowEnd = this.constrainHeight(Math.min(this.rowSize + colAxis.dims - 1, index));
};
...
PivotTable.prototype.doStickyColumns = function() {
    return this.layout.stickyColumnDimension;
};

PivotTable.prototype.doStickyRows = function() {
    return this.layout.stickyRowDimension;
};

PivotTable.prototype.doShowHierarchy = function() {
    return !!this.layout.showHierarchy;
};
...
PivotTable.prototype.getPrettyHtml = function(htmlValue) {
    return this.optionConfig.prettyPrint(htmlValue, this.layout.digitGroupSeparator);
};
...
PivotTable.prototype.getViewportWidthIndex = function() {
    return Math.floor(this.viewportWidth / this.cellWidth) + 1;
};

PivotTable.prototype.getViewportHeightIndex = function() {
    return Math.floor(this.viewportHeight / this.cellHeight) + 1;
};
...
PivotTable.prototype.getTopPadding = function() {
    return (this.rowStart + (this.rowStart > 0 && !this.options.skipTitle ? 1 : 0)) * this.cellHeight;
};
...
PivotTable.prototype.getRowSize = function() {
    ...
    if (this.doColSubTotals()) size += this.rowAxis.size / this.rowUniqueFactor;
    if (this.doColTotals())    size += 1;
    return size;
};

PivotTable.prototype.getColumnSize = function() {
    let size = this.colAxis.size;
    if (this.doRowSubTotals()) size += this.colAxis.size / this.colUniqueFactor;
    if (this.doRowTotals())    size += 1;
    return size;
};
...
PivotTable.prototype.getUniqueFactor = (xAxis) => {
...
PivotTable.prototype.getCrossAxisLabel = function() {
    ...
    return rowAxisLabel + (colAxisLabel ? '&nbsp;/&nbsp;' + colAxisLabel : '');
};

PivotTable.prototype.getLegendSetId = function(id) {
    return this.response.metaData.items[id].legendSet;
};
...
PivotTable.prototype.buildTableRow = function(rowIndex, columnStart, columnEnd) { 
    ...
    let valueTable = this.buildValueRow(rowIndex, columnStart, columnEnd - 1),
...
PivotTable.prototype.buildRowAxisColumn = function(columnIndex, rowStart, rowEnd) {
    if (this.rowAxis.dims < columnIndex) return [];
    ...
    for (let i = 0, y = rowStart; y < rowEnd; i++, y++) {
        ...
        if (this.isRowSubTotal(y)) {
            column[i] = DimensionSubTotalCell('&nbsp;', this.rowAxis.dims - columnIndex, 1, true, this.columnStart !== columnIndex);
            continue;
        }

        if (this.isRowGrandTotal(y)) {
            if (this.columnStart === columnIndex) column[i] = DimensionGrandTotalCell('Total', this.rowAxis.dims - columnIndex, 1, false, false);
            else                                  column[i] = DimensionSubTotalCell('&nbsp;', this.rowAxis.dims - columnIndex, 1, true, true);
            continue;
        }

        let axisObject = this.getRowAxisObject(columnIndex, this.normalizeRowIndex(y));
        column[i] = RowAxisCell(axisObject, this.response, this.doShowHierarchy(), !(axisObject.rowSpan || axisObject.colSpan));
    }
    return column;
};

PivotTable.prototype.buildRowAxisRow = function(rowIndex, columnStart) {
    if (this.rowAxis.dims < columnStart) return [];
    ...
    for (let i = 0, x = columnStart; x < this.rowAxis.dims; i++, x++) {
        let axisObject = this.getRowAxisObject(x, rowIndex);
        row[i] = RowAxisCell(axisObject, this.response, this.doShowHierarchy(), !(axisObject.rowSpan || axisObject.colSpan));
    }
...
PivotTable.prototype.buildColumnAxis = function(columnStart, columnEnd, rowStart) {
    if (!this.colAxis.type) return this.buildCornerAxisRow(0, 0);
...
PivotTable.prototype.buildColumnAxisRow = function(rowIndex, columnStart, columnEnd) {
    if (this.colAxis.dims < rowIndex) return [];
    ...
    for (i = row.length, x = columnStart + row.length; x < columnEnd; i++, x++) {
        
        if (this.isColumnSubTotal(x)) {
            if (i === 0) row[i] = DimensionSubTotalCell('&nbsp;', 1, this.colAxis.dims - rowIndex, true, false);
            else         row[i] = DimensionSubTotalCell('&nbsp;', 1, this.colAxis.dims - rowIndex, true, true);
            continue;
        }

        if (this.isColumnGrandTotal(x)) {
            if (rowIndex === t.rowStart) row[i] = DimensionGrandTotalCell('Total', 1, this.colAxis.dims - rowIndex, this.doSortableColumnHeaders(), false);
            else                         row[i] = DimensionSubTotalCell('&nbsp;', 1, this.colAxis.dims - rowIndex, true, true);
            continue
        }

        let axisObject = this.getColumnAxisObject(rowIndex, this.normalizeColumnIndex(x));
        row[i] = ColumnAxisCell(axisObject, this.response, this.doShowHierarchy(), !(axisObject.rowSpan || axisObject.colSpan), this.getRowSortId(rowIndex));
    }
    return row;
};
    
PivotTable.prototype.buildColumnAxisColumn = function(columnIndex, rowStart) {
    if (this.colAxis.dims < rowStart) return [];
    ...
    for (let i=0, y=rowStart; y < this.colAxis.dims; i++, y++) {
        let axisObject = this.getColumnAxisObject(y, columnIndex);
        column[i] = ColumnAxisCell(axisObject, this.response, this.doShowHierarchy(), !(axisObject.rowSpan || axisObject.colSpan), this.getRowSortId(columnIndex));
    }
...
PivotTable.prototype.buildCornerAxisRow = function(rowIndex, columnStart) {
    ...
    if (rowIndex === this.colAxis.dims - 1) {
        let i = 0;
        for (let x=columnStart; x < this.rowAxis.dims - 1; i++, x++) {
            row[i] = DimensionLabelCell(this.getRowAxisLabel(i));
        }
        row[i] = DimensionLabelCell(this.getCrossAxisLabel());
        return row;
    }
    ...
    row[i] = DimensionLabelCell(this.getColumnAxisLabel(rowIndex));
    return row;
};
...
PivotTable.prototype.itransformColPercentage = function() {
...
PivotTable.prototype.itransformRowPercentage = function() {
    const table = this.table;
    for (let i = 0; i < table.length; i++) {
        for (let j = 0; j < table[i].length; j++) {
            if (!table[i][j].empty) {
...
PivotTable.prototype.buildTableFilter = function(span) {
    ...
    let text = this.layout.filters.getRecordNames(false, this.layout.getResponse(), true);

    return [
        [this.buildHtmlCell(FilterCell(text, this.getTopBarSpan(span)))]
    ];
};
...
PivotTable.prototype.buildTableTitle = function(span) {
    ...
    return [
        [this.buildHtmlCell(FilterCell(this.layout.title, this.getTopBarSpan(span)))]
    ];
};
...
PivotTable.prototype.buildHtmlCell = function(config) {
    ...
    // html value
    let htmlValue = this.getHtmlValue(config);
    ...
    if (config.isValue && this.legendSet) {
        ...
        if (this.doLegendDisplayByDataItem() && config.dxId && this.response.metaData.items[config.dxId].legendSet) {
            legends = this.appManager.getLegendSetById(getLegendSetId(config.dxId)).legends;
        }
...
PivotTable.prototype.buildHtmlTableRows = function(htmlArray, start, end) {
    if (this.doTableClipping()) return this.buildHtmlTableRowsWithPaddng(htmlArray, start, end);
...
PivotTable.prototype.buildHtmlTable = function(htmlArray) {
    ...
    cls += this.layout.displayDensity ? ' displaydensity-' + this.layout.displayDensity : '';
    cls += this.layout.fontSize       ? ' fontsize-' + this.layout.fontSize : '';
...
PivotTable.prototype.getAdjustedColSpan = function(cell, y) {
    if (cell.children) {
        return cell.oldestSibling.children * this.colAxis.span[y + 1] - cell.siblingPosition;
    }
...
PivotTable.prototype.getAdjustedRowSpan = function(cell, x) {
    if (cell.children) {
        return cell.oldestSibling.children * this.rowAxis.span[x + 1] - cell.siblingPosition;
    }
